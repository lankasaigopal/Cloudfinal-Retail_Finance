...
page = st.sidebar.radio(
    "Go to",
    ["Dashboard", "Household Search", "Sample Data Pull", "CLV Analysis", "Churn Analysis", "Basket Analysis", "Data Upload"]
)
...
# ---------- Basket Analysis ----------
    ...
    st.markdown("<div class='inner-page'><h1>Basket Analysis</h1><p>Identify commonly purchased product combinations and cross-selling opportunities.</p></div>", unsafe_allow_html=True)

    st.markdown("<div class='info-card'>Basket Analysis helps identify product categories that are frequently purchased together in the same basket. These insights can support bundle offers, product recommendations, and cross-selling strategies.</div>", unsafe_allow_html=True)

    basket_df = data[["basket_num", "commodity"]].dropna().copy()

    basket_products = (
        basket_df.groupby("basket_num")["commodity"]
    ...
    for products in basket_products["commodity"]:
        if len(products) > 1:
            for pair in combinations(sorted(products), 2):
                pair_counter[pair] += 1

    top_pairs = [pair for pair in pair_counter.items() if pair[1] > 5]
    top_pairs = sorted(top_pairs, key=lambda x: x[1], reverse=True)[:10]

    if top_pairs:
        result_df = pd.DataFrame(top_pairs, columns=["Product Combination", "Frequency"])
        result_df["Product Combination"] = result_df["Product Combination"].apply(
            lambda x: f"{x[0]} + {x[1]}"
        )
        ...
        ax.bar(result_df["Product Combination"], result_df["Frequency"])
        ax.set_title("Top Product Combination for Cross-Selling")
        ax.set_xlabel("Product Combination")
        ...
        if len(result_df) == 1:
            st.info("Only one dominant product combination was found in the current data, so the chart displays one strong cross-selling opportunity.")
            
# ---------- Data Upload ----------
elif page == "Data Upload":
    st.markdown("<div class='inner-page'><h1>Data Upload</h1><p>Upload new datasets to update the dashboard.</p></div>", unsafe_allow_html=True)

    st.markdown("<div class='info-card'>Upload updated Transactions, Households, and Products datasets. The system will reload data and reflect changes across all pages.</div>", unsafe_allow_html=True)
        ...
        try:
            ...
            if uploaded_households is not None:
                with open("households.csv", "wb") as f:
                    f.write(uploaded_households.getbuffer())

            if uploaded_products is not None:
                with open("products.csv", "wb") as f:
                    f.write(uploaded_products.getbuffer())

            # Clear cache so new data loads
            st.cache_data.clear()

            st.success("Data uploaded successfully! Please refresh or navigate to Dashboard.")
